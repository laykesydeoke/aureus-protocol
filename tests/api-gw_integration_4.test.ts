...
describe("api-gw integration 4", () => {
  it("performs create-read-update cycle 4", () => {
    // Create
    const cr = simnet.callPublicFn("yield-aggregator", "create-api-gw", [Cl.uint(400)], deployer);
  ...
  it("validates ownership in workflow 4", () => {
    const cr = simnet.callPublicFn("yield-aggregator", "create-api-gw", [Cl.uint(450)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
