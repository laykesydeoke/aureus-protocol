...
describe("api-gw integration 6", () => {
  it("performs create-read-update cycle 6", () => {
    // Create
    const cr = simnet.callPublicFn("yield-aggregator", "create-api-gw", [Cl.uint(600)], deployer);
  ...
  it("validates ownership in workflow 6", () => {
    const cr = simnet.callPublicFn("yield-aggregator", "create-api-gw", [Cl.uint(650)], deployer);
    expect(cr.result).toBeOk(expect.objectContaining({ type: expect.any(Number) }));
