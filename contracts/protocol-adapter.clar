...
;; Record yield generated by protocol (owner only)
(define-public (record-protocol-yield (protocol-id uint) (yield-amount uint))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_UNAUTHORIZED)
    (asserts! (is-some (map-get? protocol-info protocol-id)) ERR_PROTOCOL_NOT_FOUND)
    (asserts! (> yield-amount u0) ERR_INVALID_PROTOCOL)

    (let ((current-perf (default-to {total-deposited: u0, total-yield-generated: u0, deposit-count: u0}
            (map-get? protocol-performance protocol-id))))
      (map-set protocol-performance protocol-id {
        total-deposited: (get total-deposited current-perf),
        total-yield-generated: (+ (get total-yield-generated current-perf) yield-amount),
        deposit-count: (get deposit-count current-perf)
      })
      (print {event: "protocol-yield-recorded", protocol: protocol-id, yield: yield-amount})
      (ok true)
    )
  )
)
